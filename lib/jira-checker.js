 ...
 */
module.exports = function JiraChecker(jiraConfig, pull_request, repo_info) {
   ...
   */
  function check() {
    var linksFromGithub = _.extend(getLinks(pull_request.body), getLinks(pull_request.title));
  ...
  function updateLinksFromJira(link) {
    return new Promise(function(resolve, reject){
      jira.getRemoteLinks(link.issue, function(err, remoteLinks){
        ...
        console.log('[JiraChecker] retrieved remoteLinks', link.issue, pull_request.html_url, err, remoteLinks);
        var remoteLink = _.find(remoteLinks, function(rl){
          return rl.object.url === pull_request.html_url;
        });
        if (remoteLink) {
          resolve();
          return;
        ...
        var linkData = {
          "object": {
            ...
            "title": 'PR: ' + repo_info.name + '#' + pull_request.number +': ' + pull_request.title,
            "icon" : {
        ...
        jira.createRemoteLink(link.issue, linkData, function(err2, ign) {
          console.log('[JiraChecker] Create remote link: ', link.issue, pull_request.html_url, err2);
          resolve();
        });
      });
    });
